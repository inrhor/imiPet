...
class CureCommand : BaseSubCommand() {

    override fun getDescription(): String {
        return TLocale.asString("COMMAND_DESCRIPTION.CURE")
    }
    ...
    override fun onCommand(sender: CommandSender, command: Command, label : String, args: Array<out String>) {
        ...
        val getCureString = args[1]
        if (!MathTool.isIntNumber(getCureString)) {
            TLocale.sendTo(sender, "NOT_INT")
            return
        }
        CureHandle.cureMoney(
            sender,
            petUUID,
            info().getPetModelId(sender, petUUID, "pets"),
            getCureString.toDouble(),
            "pets"
        )
